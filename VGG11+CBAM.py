...
#定义模型框架函数
    def vggNet(self, input):
        with tf.name_scope("VGG11_CBAM"):
            ...
            output_1 = self.cbam_block(output_1, name="vgg11_cbam_1")
            ...
            output_2 = self.cbam_block(output_2, name="vgg11_cbam_2")
            ...
            output_3 = self.conv_layer(output_pool_2, (3, 3), 128, [1, 1, 1, 1], name="conv_3", batch_normalize=True,
                                  activation=self.lrelu)
            output_3 = self.cbam_block(output_3, name="vgg11_cbam_3")
            output_4 = self.conv_layer(output_3, (3, 3), 128, [1, 1, 1, 1], name="conv_5", batch_normalize=True,
                                  activation=self.lrelu)
            output_4 = self.cbam_block(output_4, name="vgg11_cbam_4")
            ...
            output_5 = self.conv_layer(output_pool_3, (3, 3), 256, [1, 1, 1, 1], name="conv_7", batch_normalize=True,
                                  activation=self.lrelu)
            output_5 = self.cbam_block(output_5, name="vgg11_cbam_5")
            output_6 = self.conv_layer(output_5, (3, 3), 256, [1, 1, 1, 1], name="conv_8", batch_normalize=True,
                                  activation=self.lrelu)
            output_6 = self.cbam_block(output_6, name="vgg11_cbam_6")
            ...
            output_7 = self.conv_layer(output_pool_4, (3, 3), 256, [1, 1, 1, 1], name="conv_12", batch_normalize=True,
                                   activation=self.lrelu)
            output_7 = self.cbam_block(output_7, name="vgg11_cbam_7")
            output_8 = self.conv_layer(output_7, (3, 3), 256, [1, 1, 1, 1], name="conv_13", batch_normalize=True,
                                   activation=self.lrelu)
            output_8 = self.cbam_block(output_8, name="vgg11_cbam_8")
...
#定义训练函数
    def train(self):
        ...
        train_loss_list = []
        train_accuracy_list = []
        f1 = open(r"E:\CNN\VGGNet11+CBAM_train_loss", "w")
        f2 = open(r"E:\CNN\VGGNet11+CBAM_train_error", "w")
        f3 = open(r"E:\CNN\VGGNet11+CBAM_test_error", "w")
...
if __name__ == "__main__":
    model_output = SelectModel(num_classes=2,
                               ...
                               epoch_num=30,
                               batch_size=32,
                               model_path="./VGG11+CBAM")#batch_size 设置为64时会出现OOM错误

    import time, datetime

    startTime = datetime.datetime(2019, 3, 4, 19, 10)
