...
#定义模型框架函数
    def vggNet(self, input):
        with tf.name_scope("AM-VGG16"):
            ...
            output_1 = self.cbam_block(output_1, name="vgg_cbam_1")
            ...
            output_2 = self.cbam_block(output_2, name="vgg_cbam_2")
            output_pool_2 = self.max_pooling(output_2, (2, 2), (2, 2), name="max_pooling_2")
            print(output_pool_2.get_shape())

            output_3 = self.conv_layer(output_pool_2, (1, 1), 128, [1, 1, 1, 1], name="conv_3", batch_normalize=True,
                                  activation=self.lrelu)
            output_4 = self.conv_layer(output_3, (3, 3), 64, [1, 1, 1, 1], name="conv_4", batch_normalize=True,
                                  activation=self.lrelu)
            output_5 = self.conv_layer(output_4, (1, 1), 128, [1, 1, 1, 1], name="conv_5", batch_normalize=True,
                                  activation=self.lrelu)
            output_5 = self.cbam_block(output_5, name="vgg_cbam_3")
            output_pool_3 = self.max_pooling(output_5, (2, 2), (2, 2), name="max_pooling_3")
            print(output_pool_3.get_shape())

            output_6 = self.conv_layer(output_pool_3, (1, 1), 256, [1, 1, 1, 1], name="conv_6", batch_normalize=True,
                                  activation=self.lrelu)
            output_7 = self.conv_layer(output_6, (3, 3), 128, [1, 1, 1, 1], name="conv_7", batch_normalize=True,
                                  activation=self.lrelu)
            output_8 = self.conv_layer(output_7, (1, 1), 256, [1, 1, 1, 1], name="conv_8", batch_normalize=True,
                                  activation=self.lrelu)
            output_8 = self.cbam_block(output_8, name="vgg_cbam_4")
            output_pool_4 = self.max_pooling(output_8, (2, 2), (2, 2), name="max_pooling_4")
            print(output_pool_4.get_shape())

            output_9 = self.conv_layer(output_pool_4, (1, 1), 512, [1, 1, 1, 1], name="conv_9", batch_normalize=True,
                                  activation=self.lrelu)
            output_10 = self.conv_layer(output_9, (3, 3), 256, [1, 1, 1, 1], name="conv_10", batch_normalize=True,
                                   activation=self.lrelu)
            output_11 = self.conv_layer(output_10, (1, 1), 512, [1, 1, 1, 1], name="conv_11", batch_normalize=True,
                                   activation=self.lrelu)
            output_12 = self.conv_layer(output_11, (3, 3), 256, [1, 1, 1, 1], name="conv_12", batch_normalize=True,
                                   activation=self.lrelu)
            output_13 = self.conv_layer(output_12, (1, 1), 512, [1, 1, 1, 1], name="conv_13", batch_normalize=True,
                                   activation=self.lrelu)
            output_13 = self.cbam_block(output_13, name="vgg_cbam_5")
            output_pool_5 = self.max_pooling(output_13, (2, 2), (2, 2), name="max_pooling_5")
            ...
            # 55296=num_features
            fc_1 = self.fc_layer(output_22, num_features, 1024, name="fc_1", batch_normalize=True, activation=self.lrelu)
            fc_2 = self.fc_layer(fc_1, 1024, 1024, name="fc_2", batch_normalize=True, activation=self.lrelu)
            print(fc_2.name)
...
#定义训练函数
    def train(self):
        ...
        train_loss_list = []
        train_accuracy_list = []
        f1 = open(r"E:\CNN\VGGNet16+CBAM_train_loss", "w")
        f2 = open(r"E:\CNN\VGGNet16+CBAM_train_error", "w")
        f3 = open(r"E:\CNN\VGGNet16+CBAM_test_error", "w")
...
if __name__ == "__main__":
    model_output = SelectModel(num_classes=2,
                               ...
                               epoch_num=30,
                               batch_size=30,
                               model_path="./AM-VGG16")#batch_size 设置为64时会出现OOM错误

    import time, datetime

    startTime = datetime.datetime(2019, 3, 26, 17, 50)
