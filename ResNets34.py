...
#定义ResNet的inception模块
    def building_block(self, inputs, filter_size, filters, strides, name):
        with tf.variable_scope(name):
            x = 0
            if strides[1] > 1:
                x = self.conv_layer(inputs, (1, 1), filters, strides, name="conv_x", batch_normalize=True, activation=False,
                                    training=True)
...
#定义训练函数
    def train(self):
        ...
        train_loss_list = []
        train_accuracy_list = []
        f1 = open(r"E:\CNN\resNet_train_loss", "w")
        f2 = open(r"E:\CNN\resNet_train_error", "w")
        f3 = open(r"E:\CNN\resNet_test_error", "w")
...
if __name__ == "__main__":
    ...
    startTime = datetime.datetime(2019, 3, 4, 13, 40)
